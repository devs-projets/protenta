export const fetchLogs = async (params = {}, access_token: string, serreId: string) => {
  const queryString = new URLSearchParams(params).toString();
  console.log("From Journal", access_token);
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL }/logs/${serreId}?${queryString}`, {
    method: "GET",
    headers: {
